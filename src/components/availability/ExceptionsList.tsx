    ...
    <div>
      ...
      {sorted.length > 0 ? (
          ...
          {sorted.map((exc) => (
            <div key={exc.exception_id} className="flex items-center justify-between py-3">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-neutral-100 dark:bg-white/[0.04] flex items-center justify-center flex-shrink-0">
                  <Icons.Calendar className="h-4 w-4 text-slate-500 dark:text-slate-400" />
                </div>
                <div>
                  ...
                  {exc.reason && <p className="text-xs text-slate-500 dark:text-slate-400">{exc.reason}</p>}
                </div>
              </div>
              <button
                onClick={() => onRemove(exc.exception_id)}
                className="p-1.5 text-slate-400 dark:text-slate-400 hover:text-rose-500 transition-colors"
                title="Unblock"
              >
        ...
        <div className="text-center py-10">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-400">No blocked dates</p>
